...
from flask import Flask, abort, request, make_response, session, jsonify
...
class Users(Resource):
    ...
    # 1.3.1 use .get_json() to convert the request json 
        ...
        db.session.add(new_user) # this session is different from 'session' we store user data

        db.session.commit()
        # 1.3.4 Save the new users id to the session hash
        
        #import ipdb; ipdb.set_trace() #break point 
        #in the debugger, search for "session['user_id']"
...
class AuthorizedSession(Resource):
    ...
    def get(self):
        ...
        if user:
            ...
            return response
        # else raise the Unauthorized exception (Note- Unauthorized is being imported from werkzeug.exceptions)
        else:
